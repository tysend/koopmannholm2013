...
# https://cslabsfu.wordpress.com/2011/07/15/update-using-the-ez-package-in-r-for-anova/
aov <- ezANOVA(data = joined, dv = IdealLAP, wid = n,
               between = .(Meditator), between_covariates = .(Age, ActualLAP), detailed = TRUE)

unlisted <- unlist(aov)

aov_stats <- tibble(
  effect = ("Meditator"),
  F = (as.double(unlisted["ANOVA.F"])), 
  p = (as.double(unlisted["ANOVA.p"])),
  SSn = (as.double(unlisted["ANOVA.SSn"])),
  SSd = (as.double(unlisted["ANOVA.SSd"]))
) %>% 
...
knitr::kable(aov_stats, caption = "ANCOVA Statistics") 
```
